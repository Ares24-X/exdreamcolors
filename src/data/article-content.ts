...
Two of those failure patterns are worth calling out up front, because both are invisible if you only test tokens against white. First, moving dark text onto a tinted surface divides its contrast ratio by a fixed constant — which means a muted token needs **5.6:1 on white**, not 4.5:1, to be safe across a normal neutral surface stack. Second, if your body text is lighter than about 13.5:1 against white, there is **no link color that can satisfy both SC 1.4.3 and SC 1.4.1 at once** — the underline is not a style choice, it is the only compliant option. Both results are derived below.
...
Use the [Contrast Checker](/contrast-checker/) to validate your text tokens against real surfaces. For button-specific guidance, see [WCAG Contrast Checker for Buttons](/wcag-contrast-checker-for-buttons/). For dark mode token pairs, see [WCAG Contrast Checker for Dark Mode](/wcag-contrast-checker-for-dark-mode/). For the full accessibility picture — forms, charts, dark mode, and color blindness — see the [Color Accessibility Hub](/color-accessibility-hub/).`,
    sectionFlow: ["realWorldExamples", "audit_data", "over_media", "testing_methods", "code", "pro_tips", "tools"],
    auditHeading: "Surface Drift and the Link Contrast Paradox",
    chartAudit: `Two failure modes account for most of the muted-text and link problems I find in audits. Neither shows up when you test a token against white, which is why both survive design review. Every number in this section is computed from the WCAG 2.x relative-luminance formula and is regenerated by \`npm run verify:contrast\` on each build.
...
\`\`\`
ratio(text, surface) = ratio(text, #FFFFFF) / ratio(#FFFFFF, surface)
\`\`\`

I verified this across 7 text tokens × 9 surfaces. Maximum deviation was 3.6e-15 — floating-point noise. The identity holds because the +0.05 flare constant cancels when both terms share the white reference.
...
SC 1.4.1 says a link inside a paragraph must be distinguishable by something other than color alone — or, if you rely on color, it needs 3:1 against the surrounding text. SC 1.4.3 separately requires the link to hit 4.5:1 against the background. Teams treat these as two checkboxes. They are actually a constraint system, and it is often unsatisfiable.
...
| --- | --- | ---: | ---: | --- |
| blue-800 | #1E40AF | 8.7:1 | 2.0:1 | No — separation fails |
| blue-700 | #1D4ED8 | 6.7:1 | 2.7:1 | No — separation fails |
| blue-600 | #2563EB | 5.2:1 | 3.4:1 | **Yes** |
| blue-500 | #3B82F6 | 3.7:1 | 4.8:1 | No — AA fails |
| blue-400 | #60A5FA | 2.5:1 | 7.0:1 | No — AA fails |
...
Validate your own token pairs with the [Contrast Checker](/contrast-checker/). For the same drift problem on dark surfaces, where the divisor works in the opposite direction, see [WCAG Contrast Checker for Dark Mode](/wcag-contrast-checker-for-dark-mode/). For alert and validation tokens specifically, see [Form Validation Color Accessibility](/form-validation-color-accessibility/). For the full picture across forms, charts, and palettes, start at the [Color Accessibility Hub](/color-accessibility-hub/).`,
    realWorldExamples: `**Stripe keeps brand blue away from long-form body copy.** Their product pages use vivid blue (#635BFF) for accents and actions, but paragraph text sits at #3C4257 on white — a 10.0:1 ratio. That split keeps the brand recognizable without forcing blue to do a reading job it was not designed for.

**Amazon uses orange as an action signal, not as paragraph text.** Orange buttons (#FF9900 text on dark) can work because they are large, bold, and surrounded by neutral surfaces. The same hue at 14px on white scores only 2.1:1 — a hard fail. Amazon's body text uses #0F1111 on white (18.9:1).
...
| Placeholder text | 4.5:1 | 4.5:1 | #9CA3AF on #FFFFFF | 2.5:1 | Common fail — most placeholders are decorative, but required-field hints are not |
| Error text | 4.5:1 | 7:1 | #B91C1C on #FFFFFF | 6.5:1 | Must also pass on tinted error backgrounds |
| Focus ring | 3:1 | 4.5:1 | #2563EB ring on #FFFFFF | 5.2:1 | WCAG 2.2 SC 2.4.13 requires ≥2px perimeter area |
...
- After dark mode changes — dark tokens drift independently from light`,
    codeSnippet: {
      label: "Token audit script — surface drift guard + link constraint solver",
...
/* ---------------------------------------------------------------
   ...
   white to be safe on EVERY surface — no re-measuring per pair.
--------------------------------------------------------------- */
const WHITE = '#FFFFFF';
...
for (const fg of textTokens) {
  ...
  if (onWhite >= min && onWhite < required) {
    console.log(
      \`⚠ \${fg.name} passes on white (\${onWhite.toFixed(2)}:1) but needs \` +
...
if (!colorOnlyLinkIsPossible(bodyHex)) {
  console.log(
    ...
    'No hue satisfies SC 1.4.3 + SC 1.4.1 together — underline required.'
  );
} else {
  ...
  console.log(
    ...
    proTips: [
      ...
      "Do not size muted text against white. Moving dark text onto a tinted surface divides its ratio by that surface's own contrast against white — a fixed constant, not a guess. gray-200 (#E5E7EB) has a divisor of 1.238, so a muted token needs 5.6:1 on white to stay AA-compliant everywhere. Tailwind's gray-500 (#6B7280) measures 4.8:1 on white and 3.9:1 on gray-200. Use gray-600 (#4B5563) at 7.6:1 instead.",
      "Alert text needs the -700 step, not -600. Every semantic -600 token fails on its own tinted background: #DC2626 drops to 4.4:1 on #FEF2F2, #059669 to 3.6:1 on #ECFDF5, #D97706 to 3.1:1 on #FFFBEB. Use #B91C1C, #047857, and #B45309 for text inside alerts, and keep the -600 shades for icons and borders where SC 1.4.11's 3:1 threshold applies.",
      "Pin your published ratios to a build-time check. Hand-written contrast numbers in docs and design-system tables drift every time a token changes. A prebuild script that recomputes each printed ratio from its two hex values and fails the build on mismatch takes an hour to write and catches errors no reviewer will spot.",
      "Links must be distinguishable from surrounding body text by either 3:1 contrast ratio OR a non-color cue. Do not try to win this with color: unless your body text measures at least 13.5:1 against white (roughly #2E2E2E or darker), no link color can satisfy SC 1.4.3 and SC 1.4.1 simultaneously. With body at #374151 the best any link can reach is 3.44:1 on white. Underline in-paragraph links and pick the link color purely for the 4.5:1 background requirement.",
      ...
      "Pre-ship text contrast checklist: (1) All body text ≥7:1 on actual surfaces (2) Muted text ≥5.6:1 on white so it survives every tinted surface in the system (3) In-paragraph links underlined — do not rely on color separation (4) Hover/active/focus states tested independently (5) Error/success text uses the -700 step so it passes on its own tinted background (6) Placeholder text either ≥4.5:1 or marked decorative (7) Dark mode tokens audited separately (8) Font weights below 400 boosted to 7:1 minimum (9) Real device check in outdoor sunlight (10) Published ratio tables verified by a build-time script, not by hand.",
    ...
    keyStat: "Two provable results from this page. First, surface drift is a fixed divisor: ratio(text, surface) equals ratio(text, white) divided by ratio(white, surface), verified to 3.6e-15 across 7 tokens × 9 surfaces — which means a muted token needs 5.6:1 on white, not 4.5:1, to survive a Tailwind-style neutral stack. Tailwind's gray-500 measures 4.8:1 on white and 3.9:1 on gray-200. Second, a color-only in-paragraph link is mathematically impossible unless body text measures at least 13.5:1 against white; with the common gray-700 body (#374151) the best any link can achieve is 3.44:1, so the underline is the only compliant option.",
    toolsMention: ["contrast-checker", "color-picker", "palette-generator"],
  },
...
| Slider track | #E5E7EB on #FFFFFF | 1.2:1 | #9CA3AF still fails at 2.5:1 — use #767E8C (4.1:1) or add a label |
| Icon button (no text) | #9CA3AF icon on #FFFFFF | 2.5:1 | Darken to #6B7280 (4.8:1) |
| Pie chart adjacent slices | #3B82F6 vs #60A5FA | 1.5:1 | #2563EB vs #93C5FD is still only 2.9:1 — use #1E40AF vs #93C5FD (4.8:1) or add borders |
