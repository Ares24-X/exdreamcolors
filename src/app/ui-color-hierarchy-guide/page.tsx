...
export default function UiColorHierarchyGuidePage() {
  return (
    <main className="max-w-4xl mx-auto px-4 py-12">
      ...
      <section className="mb-10"><h2 className="text-2xl font-bold text-slate-900 mb-4">How Hierarchy Works</h2><div className="prose prose-slate max-w-none text-slate-700" dangerouslySetInnerHTML={{ __html: markdownToHtml(content.realWorldExamples) }} /></section>
      ...
      <section className="mb-10"><h2 className="text-2xl font-bold text-slate-900 mb-4">Review Checklist</h2><ul className="space-y-3">{content.proTips.map((tip, index) => (<li key={index} className="flex gap-3 text-slate-700"><span className="text-blue-500 font-bold">▸</span><span>{tip}</span></li>))}</ul></section>
