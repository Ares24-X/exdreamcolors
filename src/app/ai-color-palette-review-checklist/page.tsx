...
export default function AiColorPaletteReviewChecklistPage() {
  return (
    <main className="max-w-4xl mx-auto px-4 py-12">
      ...
      <section className="mb-10"><h2 className="text-2xl font-bold text-slate-900 mb-4">Where AI Palettes Usually Break</h2><div className="prose prose-slate max-w-none text-slate-700" dangerouslySetInnerHTML={{ __html: renderMarkdown(content.realWorldExamples) }} /></section>
      <section className="mb-10"><h2 className="text-2xl font-bold text-slate-900 mb-4">Checklist</h2><ul className="space-y-3">{content.proTips.map((tip, index) => (<li key={index} className="flex gap-3 text-slate-700"><span className="text-blue-500 font-bold">▸</span><div className="min-w-0 flex-1" dangerouslySetInnerHTML={{ __html: renderTipMarkdown(tip) }} /></li>))}</ul></section>
